...
class FilmModel {
  ...
  static getAllFilm = () => {
    const query =
      'SELECT f.id, filmName, originName, thurmUrl, posterUrl, slug, description, status, episodeCurrent, episodeTotal, released, countryName FROM ' +
      this.tableName +
      ' f, `dbo.country` c where f.country = c.id ORDER BY episodeCurrent DESC'
    ...
    return new Promise((resolve, reject) => {
      connection.query(query, async (error, results) => {
        ...
        const filmsPromise = films?.map(async (f) => {
          let queryCate =
            'SELECT categoryName FROM `dbo.film_category` fc, `dbo.category` c WHERE fc.categoryId = c.id AND fc.filmId = ' +
            f.id
  ...
  static createFilm = async (data) => {
    ...
    return new Promise(async (resolve, reject) => {
      connection.query(query, values, (err, results) => {
        ...
        resolve(
          new Promise(async (resolveCategories, rejectCategories) => {
            categories.map(async (category) => {
              await connection.query(
                `INSERT INTO \`dbo.film_category\`(filmId, categoryId) VALUES(${filmId}, ${category})`,
                (err) => {
    ...
    return new Promise(async (resolve, reject) => {
      connection.query(queryCF, async (err, result) => {
        ...
        if (categoriesToRemove.length > 0) {
          const removeQuery = `DELETE FROM \`dbo.film_category\` WHERE filmId = ${
            data.id
          } AND categoryId IN (${categoriesToRemove.join(',')})`
